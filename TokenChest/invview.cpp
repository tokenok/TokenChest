...
POINT get_item_inv_loc_from_pt(string store, POINT pt, const InvData& invdata) {
	if (store == "inventory") {
		...
		vector<RECT> body = {
			{132, 4, 132 + 29 * 2, 4 + 29 * 2}, {205, 30, 205 + 29, 30 + 29}, {134, 74, 134 + 29 * 2, 74 + 29 * 3}, {17, 45, 17 + 29 * 2, 45 + 29 * 4},
			{248, 45, 248 + 29 * 2, 45 + 29 * 4}, {91, 175, 91 + 29, 175 + 29}, {205, 175, 205 + 29, +175 + 29}, {134, 175, 134 + 29 * 2, 175 + 29},
			{251, 175, 251 + 29 * 2, 175 + 29 * 2}, {18, 176, 18 + 29 * 2, 176 + 29 * 2}, {17, 45, 17 + 29 * 2, 45 + 29 * 4}, {248, 45, 248 + 29 * 2, 45 + 29 * 4}
		};
		...
		vector<RECT> merc = {{132, 4, 132 + 29 * 2, 4 + 29 * 2}, {134, 74, 134 + 29 * 2, 74 + 29 * 3}, {17, 45, 17 + 29 * 2, 45 + 29 * 4}, {248, 45, 248 + 29 * 2, 45 + 29 * 4}};
								...
								case 1:{
									for (UINT i = 0; i < 9; i++) {
										if (characterpage == MENU_WAYPOINT_NORM ? chardata->wp_norm[i] == '1'
											...
											: false) {
											rcicon = {x, h(i), icon->GetWidth() + x, icon->GetHeight() + h(i)};
											DrawPicture(hDCmem, icon, &rcicon);

											rctext = {80, h(i) + 8, img->GetWidth(), img->GetHeight() + 8};
								...
								case 2:{
									for (UINT i = 9; i < 18; i++) {
										if (characterpage == MENU_WAYPOINT_NORM ? chardata->wp_norm[i] == '1'
											...
											: characterpage == MENU_WAYPOINT_HELL ? chardata->wp_hell[i] == '1'
											: false) {
											rcicon = {x, h(i - (9 * (selectedsubpage - 1))), icon->GetWidth() + x, icon->GetHeight() + h(i - (9 * (selectedsubpage - 1)))};
											DrawPicture(hDCmem, icon, &rcicon);

											rctext = {80, h(i - (9 * (selectedsubpage - 1))) + 8, img->GetWidth(), img->GetHeight() + 8};
											DrawText(hDCmem, str_to_wstr(wpnames[i]).c_str(), wpnames[i].size(), &rctext, DT_VCENTER | DT_NOCLIP);
										}
									}
									break;
								...
								case 5:{
									for (UINT i = 30; i < 39; i++) {
										if (characterpage == MENU_WAYPOINT_NORM ? chardata->wp_norm[i] == '1'
											...
											: characterpage == MENU_WAYPOINT_HELL ? chardata->wp_hell[i] == '1'
											: false) {
											rcicon = {x, h(i - 30), icon->GetWidth() + x, icon->GetHeight() + h(i - 30)};
											DrawPicture(hDCmem, icon, &rcicon);

											rctext = {80, h(i - 30) + 8, img->GetWidth(), img->GetHeight() + 8};
											DrawText(hDCmem, str_to_wstr(wpnames[i]).c_str(), wpnames[i].size(), &rctext, DT_VCENTER | DT_NOCLIP);
										}
									}
									break;
